...
class ATMAuthenticationDatabase(AbstractDatabase):
    ...
    def get_labels_and_embs(self):
        ...
        # only get face of a user
        faces = self.mongodb_face.find()
        nrof_faces = faces.count()
        insert_count = 0
        for face in faces:
            insert_count += 1
            new_embs = np.array(face['embeddings'])
            label = str(face['aliasId'])
            print('inserting %s/%s' % (insert_count, nrof_faces), end='\r')
            new_labels = [label] * len(new_embs)
            if new_embs.shape[0] == 0:
                # skip no embedding record
                continue
            embs.append(new_embs)
    ...
    def delete_user_by_aliasID(self, aliasId):
        res = self.mongodb_face.delete_one({'aliasId': aliasId})
...
class RetentionDashboardDatabase(AbstractDatabase):
    ...
    def setup(self):
        ...
        self.mongodb_pedestrianinfo = self.mongodb_db[Config.MongoDB.PEDESTRIANINFO_COLS_NAME]
    ...
    def get_biggest_tracker_number(self):
        try:
            _number = self.mongodb_faceinfo.find({}).sort('trackId', -1)[0]['trackId']
            print('Biggest track id number', _number)
            return _number
        except:
            print('Biggest track id number', 0)
            return 0
    ...
    def insert_new_pedestrian(self, **args):
        self.mongodb_pedestrianinfo.remove({'imageId': args.get('imageId')})
        self.mongodb_pedestrianinfo.insert_one({
            'trackId':
            args.get('trackId'),
            'image':
            args.get('image'),
    ...
    def find_face_by_track_id(self, track_id):
        ...
        is_registered = False
        if cursors.count() != 0:
            is_registered = cursors[0]['isRegistered']
            for cursor in cursors:
    ...
    def find_face_id_by_represent_image_id_in_faceinfo(self, represent_image_id):
        cursor = self.mongodb_faceinfo.find_one({
            'imageId': represent_image_id})
        if cursor is not None and 'faceId' in cursor:
            return cursor['faceId']
        return None
    ...
    def find_time_stamp(self):
        ...
        if undone.count() > 0:
            ...
            if time is not None and int(undone[0]['recordTimestamp']) > int(time['recordTimestamp']):
                time = undone[0]
        if time != None:
    ...
    def timestamp_insert(self, **kwargs):
        self.mongodb_timestamp.insert_one(kwargs)

    def timestamp_find(self, **kwargs):
        return self.mongodb_timestamp.find_one(kwargs)

    def timestamp_finds(self, **kwargs):
        return self.mongodb_timestamp.find(kwargs)


class RetentionDashboardBatchQueryDatabase(RetentionDashboardDatabase):
    # TODO: Refactor
    ...
    def find_face_id_by_image_id_in_faceinfo(self, image_ids):
        ...
        for key in cursor:
            for i in range(count[key['imageId']]):
                face_ids.append(key['faceId'])
        if len(face_ids) != len(image_ids):
            raise Exception('len of face ids unmatched len of image ids')
        return face_ids
