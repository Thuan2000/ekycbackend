...
class ImageSocketDynamicPutResult(ImageSocket):
    ...
    def __init__(self, hostname, port, max_queue_size=50):
        ...
        print('Connecting on %s:%s' %(self.hostname, self.port))
        if port == 80:
            self.socket = SocketIO(self.hostname, verify=False)
        else:
            self.socket = SocketIO(self.hostname, self.port, verify=False)
...
class RetentionDashboardSocket():
    ...
    def __init__(self, hostname, port):
        ...
        if Config.Mode.PRODUCTION:
            self.socket = SocketIO(self.hostname, verify=False) #, self.port)
...
@catch_disconnect_all_class_methods
class CombinedRetentionDashboardSocket():
    ...
    def send_result(self, **kwargs):
        ...
        self.retention_dashboard_namespace.emit(Config.Socket.NEW_FACE_EVENT[event], image_id)
