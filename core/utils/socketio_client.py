...
class ImageSocket():
    ...
    def __init__(self, hostname, port):
        ...
        self.sio.connect('{}:{}'.format(self.hostname, self.port))
        self.image_namespace = ImageNamespace(Config.Socket.IMAGE_NAMESPACE)
        self.sio.register_namespace(self.image_namespace)
        self.image_namespace.queue = self.queue
        self.sio.start_background_task(self.crawl_image)
...
class ImageNamespace(socketio.ClientNamespace):
...
class ImageSocketDynamicPutResult(ImageSocket):
    ...
    def __init__(self, hostname, port, max_queue_size=50):
        ...
        self.sio = socketio.Client()
        self.sio.connect('{}:{}'.format(self.hostname, self.port))
        self.image_namespace = AliasIdImageNamespace(Config.Socket.IMAGE_NAMESPACE)
        self.sio.register_namespace(self.image_namespace)
...
class RetentionDashboardSocket():
    ...
    def __init__(self, hostname, port):
        ...
        if Config.Mode.PRODUCTION:
            self.sio = socketio.Client()
            self.sio.connect(self.hostname)
            self.retention_dashboard_namespace = RetentionDashboardNamespace(Config.Socket.RETENTION_DASHBOARD_NAMESPACE)
            self.sio.register_namespace(self.retention_dashboard_namespace)
...
class RetentionDashboardNamespace(socketio.ClientNamespace):
...
class RetentionDashboardBlacklistSocket(RetentionDashboardSocket):
    ...
    def __init__(self, hostname, port):
        ...
        self.hostname = hostname
        self.port = port
        self.update_blacklist = Value(c_bool, True)

        if Config.Mode.PRODUCTION:
            self.sio = socketio.Client()
            self.sio.connect(self.hostname)
            self.retention_dashboard_namespace = RetentionDashboardBlacklistNamespace(Config.Socket.RETENTION_DASHBOARD_NAMESPACE)
            self.sio.register_namespace(self.retention_dashboard_namespace)
            self.retention_dashboard_namespace.update_blacklist = self.update_blacklist
            
            self.sio.start_background_task(self.listen)
    ...
    def get_update_blacklist(self):
        status = self.update_blacklist.value
        if status:
            self.update_blacklist = False
        return status
...
class RetentionDashboardBlacklistNamespace(RetentionDashboardNamespace):
    ...
    def on_update_pedestrian_blacklist(self, *args):
        ...
        self.update_blacklist.value = True
