...
import time
from pipe.tube import Tube
from pipe.task import Task
...
class Worker(multiprocessing.Process):
    ...
    def __init__(self):
        # Just set them up for initialization
        self._final_conn = None
        self._final = None
    ...
    def setup(
            ...
            final_conn, # Final pipe connection 
            final, # Is it the final worker
    ...
    def run(self):
        ...
        self.doInit(final_conn=self._final_conn, final=self._final)
    ...
    def doTask(self, task):
        """
        The implementation of subtaskes can publish the output result in one of two ways,
        either by
        1) calling :meth:`putResult` and returning ``None``, or
        2) returning the result (other than ``None``).
        Default behavior: delegate task to next stage

        """
        worker_time_start = time.time()
        if task.type == Task.Frame:
            self.doFrameTask(task)
        if task.type == Task.Face:
            self.doFaceTask(task)
        elif task.type == Task.Event:
            self.doEventTask(task)
        self.worker_time += time.time() - worker_time_start

    def doFaceTask(self, task):
        """
        Implement this method to hanle task with face
        Default: Delegate task to next staget
        """
        self.putResult(task)

    def doFrameTask(self, task):
        """
        Implement this method in the subclass to handle frame task
        """
        self.putResult(task)

    def doEventTask(self, task):
        """
        Implement this method in the subclass to handle event
        Event by default delegate to next stage, so putResult anyway
        """
        self.putResult(task)

    # TODO: Put more task here as the project grow

    def doInit(self, final_conn=None, final=False):
    ...
    def doFinish(self):
        """
        Do any task to clear up worker here
        """
        pass

    def doReport(self):
        """
        Report any worker statistic here
        """
        pass